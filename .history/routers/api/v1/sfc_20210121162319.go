...
func GetWorkOrders(c *gin.Context) {
	...
	if detail == "true" {
		c.JSON(http.StatusOK, FindWorkOrdersInfo()) //之後須加上by id功能
		return
	}

	//get all workorders
	var wos []model.WorkOrder
	err := FindWorkOrders(nil, &wos)
	if err != nil {
...
func GetWorkOrdersById(c *gin.Context) {
	workorderId := c.Param("workorderId") //取得URL中参数
	var wos []model.WorkOrder
	err := FindWorkOrdersById(workorderId, &wos)
	if err != nil {
		c.JSON(http.StatusInternalServerError, err)
	}
	c.JSON(http.StatusOK, wos)
