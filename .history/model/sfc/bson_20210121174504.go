...
func (c *StatsInfo) SetBSON(raw bson.Raw) error {
	...
	statsInfo := new(newStatsInfo)

	bsonErr := raw.Unmarshal(statsInfo)
	if bsonErr != nil {
		return bsonErr
	}
	util.PrintJson(statsInfo)

	statsInfo.RealCompletedRate = (statsInfo.CompletedQty / statsInfo.Quantity) * 100
